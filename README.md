...
- **设置**：GitHub 令牌、`owner/repo`、分支、草稿索引与配图目录
- **去重**：IndexedDB 记下已暂存的 `id`，卡片置灰；可单独清空发布标记或草稿列表（配置不动）

### 📝 填入发布页
- **只暂存、不发布**：只点白色「暂存离开」，绝不点红色「定时发布 / 发布」
- **标题 / 正文**：标题取 `wechat_title`；正文为关键词摘要、回复词和话题行
- **必选话题**：`#图美AI` `#ChatGPT` `#美图提示词`，再加上 JSON 里的 `keywords`
- **合集**：自动选 **ChatGPT美图**
- **定时**：约 2 小时后～未来 14 天、白天 10:00–20:00 随机写入（仍只点暂存离开）
...
- **不刷新发布页**：灌图 / 填文 / 暂存拆步，按内容多少停顿；已在 `/publish` 上不再整页 reload
...
3. 流程：拉图 → 上传图文 → 填标题/正文/话题/合集/定时 → 点「暂存离开」→ 标记已处理
4. 设置里可打开 **每天自动发布 5 篇**（实际仍是暂存离开，不会点发布）
...
- `src/lib/dom-inject.ts` — 标题、正文、合集、定时、暂存离开
